...
try:
    ...
    def callback_inline(call):
        if call.data[-2]  in '♠️♥️♣️♦️':
            ...
            if call.from_user.id != game.who_turn:
                ...
                elif game.check_turn(call.data, call.from_user.id) == 'Перевод козырем':
                    pass
                else:    
    ...
    def update_message(game):
        ...
        bot.edit_message_text(chat_id= game.host_id, message_id = game.host_message_id, text= message_for_host(game), reply_markup =kb_host )
        bot.edit_message_text(chat_id= game.guest_id, message_id = game.guest_message_id, text= message_for_guest(game), reply_markup = kb_guest)
